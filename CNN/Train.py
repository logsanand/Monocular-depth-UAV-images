...
from model_test_cnn import *
from data_test3 import *
...
parser.add_argument('--output_directory',          type=str,   help='output directory for test disparities, if empty outputs to checkpoint folder', default='/data/madhuanand/codes/Msc_project/codes/dualcnn_new/')
parser.add_argument('--log_directory',             type=str,   help='directory to save checkpoints and summaries', default='/data/madhuanand/codes/Msc_project/codes/dualcnn_new/')
...
def train(params):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        #values = [args.learning_rate/2, args.learning_rate / 4, args.learning_rate / 8]
...
#	    print("Step - {}".format(step))
            ...
            _, loss_value,disparities_train,left_train= sess.run([apply_gradient_op, total_loss,disp_left,left_image])
            #disparities_train=sess.run(disp_left)
            ...
            if step and step % 138800 == 0:
                np.save(args.log_directory + '/disparities'+'/disparities_train_last.npy', disparities_train.squeeze())
                np.save(args.log_directory +'/disparities'+ '/left_train_'+str(step)+'.npy', left_train.squeeze())
...
def test(params):
    ...
    os.mkdir(args.log_directory + args.model_name+'2')
    cnt = 0
    while cnt < num_test_samples:
        ...
        for d in disp:
            if cnt < num_test_samples:
                ...
                #plt.imshow(images)
                #plt.show()
                plt.imsave(args.log_directory +args.model_name + '2'+ '/img_' + str(cnt) + '.png' ,images)
                cnt += 1
    print('done.')
