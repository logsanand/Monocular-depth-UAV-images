...
class MonodepthDataloader(object):
    ...
    def __init__(self, data_path, filenames_file, params, dataset, mode):
        ...
        if mode == 'test' and not self.params.do_stereo:
            ...
            tf.print(left_image_path,[left_image_path])
            right_image_path = tf.string_join([self.data_path, split_line[1]])
            left_image_o  = self.read_image(left_image_path)
            right_image_o = self.read_image(right_image_path)
        else:
            left_image_path  = tf.string_join([self.data_path, split_line[0]])
            #print(tf.print(left_image_path,[left_image_path]))
            #print(split_line[0])
            #print("hai")
            right_image_path = tf.string_join([self.data_path, split_line[1]])
            left_image_o  = self.read_image(left_image_path)
        ...
        if mode == 'train':
            ...
            #dx,_=tf.image.image_gradients(left_image)
            #_,dy=tf.image.image_gradients(left_image)
            #dx1,_=tf.image.image_gradients(right_image)
            #_,dy1=tf.image.image_gradients(right_image)
            ...
            do_flip = tf.random_uniform([], 0, 1)
            left_image  = tf.cond(do_flip > 2, lambda: tf.image.flip_left_right(left_image_o), lambda: left_image_o)
            #right_image = tf.cond(do_flip > 2, lambda: tf.image.flip_left_right(left_image_o),  lambda: right_image_o)

            # randomly augment images
            #do_augment  = tf.random_uniform([], 0, 1)
            #left_image, right_image = tf.cond(do_augment > 2, lambda: self.augment_image_pair(left_image, right_image), lambda: (left_image, right_image))
            ...
            #min_after_dequeue = 2048
            #capacity = min_after_dequeue + 4 * params.batch_size
            self.left_image_batch = tf.train.batch([left_image], params.batch_size)
            ...
            if mode =='test' and self.params.do_stereo:
                ...
                do_flip = tf.random_uniform([], 0, 1)
                #left_image  = tf.cond(do_flip > 2, lambda: tf.image.flip_left_right(left_image_o), lambda: left_image_o)
                right_image = tf.cond(do_flip > 2, lambda: tf.image.flip_left_right(right_image_o),  lambda: right_image_o)
                #left_image.set_shape( [None, None, 3])
                right_image.set_shape([None, None, 3])
    ...
    def read_image(self, image_path):
        ...
        file_cond = tf.equal(file_extension, 'jpeg')
        ...
        #image  = tf.cond(file_cond, lambda: tf.image.decode_jpeg(image,name='jpeg_reader'), lambda: tf.image.decode_png(tf.read_file(image_path)))
